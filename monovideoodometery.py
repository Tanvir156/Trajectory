...
def get_absolute_scale(self):
    '''
    Used to get scale of translation for when we move
    from one frame to another
    '''

    pose_line = self.pose[self.id]
    pose_line = [float(i) for i in pose_line.split(" ")]
    pose_T = np.array(pose_line[:3]).reshape(3,1)

    if self.id == 0:
        self.initial_t = pose_T
        return 1

    else:
        self.current_t = pose_T
        absolute_scale = np.sqrt(np.sum(np.square(self.current_t - self.initial_t)))
        self.initial_t = self.current_t
        return absolute_scale


def process_frame(self):
    '''Method used to process the next frame from the folder
       This method contains a number of sub-methods used to extract
       features and perform optical flow
    '''

    frame_file = os.path.join(self.file_path, "{:06d}.png".format(self.id))

    # Load the image frame
    self.current_frame = cv2.imread(frame_file)

    # Convert to grayscale
    self.current_frame = cv2.cvtColor(self.current_frame, cv2.COLOR_BGR2GRAY)

    if self.id == 0:
        # If first frame, initialize the p0 array for
        # use with optical flow in subsequent frames
        self.p0 = self.detect(self.current_frame)
        self.n_features = len(self.p0)

    else:
        # Process visual odometery between frames
        self.visual_odometery()
        self.n_features = len(self.good_new)
